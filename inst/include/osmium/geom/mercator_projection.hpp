    ...
    namespace geom {
        ...
        namespace detail {
            ...
            inline double lat_to_y(double lat) { // not constexpr because math functions aren't
                return earth_radius_for_epsg3857 * std::log(std::tan(osmium::geom::OSMIUM_PI/4 + deg_to_rad(lat)/2));
            }
            ...
            inline double y_to_lat(double y) { // not constexpr because math functions aren't
                return rad_to_deg(2 * std::atan(std::exp(y / earth_radius_for_epsg3857)) - osmium::geom::OSMIUM_PI/2);
            }
